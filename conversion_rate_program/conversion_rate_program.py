...
while(mi < 0 and attempt < 3):
    print(TRY_AGAIN)
    print('You have',3 - attempt,'tries left.')
    mi = float(input('Miles: '))
    attempt = attempt + 1
if(mi < 0 and attempt >= 3):    #if third try is still invalid, end program
    print(TERMINATE)
else:
    attempt = 1     #resets number of tries allowed
    degF = float(input('\nTemperature (F): '))
    while(degF > 1000 and attempt < 3):
        print(TRY_AGAIN)
        print('You have',3 - attempt,'tries left.')
        degF = float(input('\nTemperature (F): '))
        attempt = attempt + 1
    if(degF > 1000 and attempt >= 3):
        print(TERMINATE)
    else:
        attempt = 1
        gal = float(input('\nGallons: '))
        while(gal < 0 and attempt < 3):
            print(TRY_AGAIN)
            print('You have',3 - attempt,'tries left.')
            gal = float(input('Gallons: '))
            attempt = attempt + 1
        if(gal < 0 and attempt >= 3):
            print(TERMINATE)
        else:
            attempt = 1
            lb = float(input('\nDry pounds: '))
            while(lb < 0 and attempt < 3):
                print(TRY_AGAIN)
                print('You have',3 - attempt,'tries left.')
                lb = float(input('\nDry pounds: '))
                attempt = attempt + 1
            if(lb < 0 and attempt >= 3):
                print(TERMINATE)
            else:
                attempt = 1
                inches = float(input('\nInches: '))
                while(inches < 0 and attempt < 3):
                    print(TRY_AGAIN)
                    print('You have',3 - attempt,'tries left.')
                    inches = float(input('\nInches: '))
                    attempt = attempt + 1
                if(inches < 0 and attempt >= 3):
                    print(TERMINATE)
                else:
                    #calculations run
                    km = mi * mi_to_km
